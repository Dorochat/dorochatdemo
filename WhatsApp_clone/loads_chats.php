...
if($_POST){//check wether data has been submitted via POST method(from form)

$conversation_id=$_POST['id']; //Grab Current Conversation Id That the user has click on

$_SESSION['current_conversation_id']=$conversation_id;// store conversation ID in session so that we can use it  while exchanging replies in a conversation
...
?>
<input type="hidden" name="currentResult" id="CurrentResult" value="4">
<?php
...
?>
  <div class="Chat_id" id="<?php echo $conversation_id; ?>">
        </div>    <!--current conversation ID-->
<?php
...
<img src="img/avatar.png" style="margin: 5px; height:5px; width:85px; border-radius: 1000px;>
</div>
